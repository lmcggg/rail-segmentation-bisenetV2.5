...
class DWConvBlock(nn.Module):
    def __init__(self, in_chan, out_chan, kernel_size=3, stride=1, padding=1,  bias=False):
        ...
        self.depth_conv = nn.Sequential(
            nn.Conv2d(in_chan, in_chan, kernel_size=kernel_size, stride=stride,
...
class DetailBranch(nn.Module):
    def __init__(self):
        ...
        self.S1 = nn.Sequential(
            DWConvBlock(in_chan=3, out_chan=64, kernel_size=3, stride=2, padding=1),
            DWConvBlock(in_chan=64, out_chan=64, kernel_size=3, stride=1, padding=1),
        )
        self.S2 = nn.Sequential(
            DWConvBlock(in_chan=64, out_chan=96, kernel_size=3, stride=2, padding=1),
            DWConvBlock(in_chan=96, out_chan=96, kernel_size=3, stride=1, padding=1),
            DWConvBlock(in_chan=96, out_chan=96, kernel_size=3, stride=1, padding=1),
        )
        self.S3 = nn.Sequential(
            DWConvBlock(in_chan=96, out_chan=128, kernel_size=3, stride=2, padding=1),
            DWConvBlock(in_chan=128, out_chan=128, kernel_size=3, stride=1, padding=1),
            DWConvBlock(in_chan=128, out_chan=128, kernel_size=3, stride=1, padding=1),
        )
...
class BiSeNetV2(nn.Module):
    def __init__(self, n_classes, seghead_ratio=6):
        ...
        self.head = SegmentHead(128, seghead_ratio, 20)
        if self.training:
            self.aux2 = SegmentHead(16, seghead_ratio, 20)
            self.aux3 = SegmentHead(32, seghead_ratio, 20)
            self.aux4 = SegmentHead(64, seghead_ratio, 20)
            self.aux5_4 = SegmentHead(128, seghead_ratio, 20)
